...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	reporter.SetThinkingFromPayload(bodyForUpstream)
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		if decErr != nil {
			...
			return resp, statusErr{code: httpResp.StatusCode, msg: msg, retryAfter: helps.ParseRetryAfter(httpResp, nil)}
		}
...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		if decErr != nil {
			...
			msg := fmt.Sprintf("failed to decode error response body: %v", decErr)
			helps.LogWithRequestID(ctx).Warn(msg)
			return nil, statusErr{code: httpResp.StatusCode, msg: msg, retryAfter: helps.ParseRetryAfter(httpResp, nil)}
		}
		b, readErr := helps.LimitedReadAll(errBody)
		if readErr != nil {
			helps.RecordAPIResponseError(ctx, e.cfg, readErr)
...
func (e *ClaudeExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if decErr != nil {
			...
			msg := fmt.Sprintf("failed to decode error response body: %v", decErr)
			helps.LogWithRequestID(ctx).Warn(msg)
			return cliproxyexecutor.Response{}, statusErr{code: resp.StatusCode, msg: msg, retryAfter: helps.ParseRetryAfter(resp, nil)}
		}
		b, readErr := helps.LimitedReadAll(errBody)
		if readErr != nil {
			helps.RecordAPIResponseError(ctx, e.cfg, readErr)
		...
		return cliproxyexecutor.Response{}, statusErr{code: resp.StatusCode, msg: string(b), retryAfter: helps.ParseRetryAfter(resp, b)}
	}
