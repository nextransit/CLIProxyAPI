...
func TestCodexExecutorExecuteStripsPreviousResponseID(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
	...
	_, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "gpt-5.4",
		Payload: []byte(`{"model":"gpt-5.4","previous_response_id":"resp_1","input":"hello"}`),
	}, cliproxyexecutor.Options{
		SourceFormat: sdktranslator.FromString("openai-response"),
		Stream:       false,
	})
	...
	if gjson.GetBytes(gotBody, "previous_response_id").Exists() {
		t.Fatalf("previous_response_id was forwarded upstream; payload=%s", gotBody)
	}
}

func TestCodexExecutorExecuteStripsPayloadSystemInstruction(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
			...
			Override: []config.PayloadRule{
				{
					...
					Params: map[string]any{"system_instruction": "do not forward"},
				},
			},
		},
	})
	...
	if gjson.GetBytes(gotBody, "system_instruction").Exists() {
		t.Fatalf("system_instruction was forwarded upstream; payload=%s", gotBody)
	}
}

func TestCodexExecutorExecuteStreamStripsPreviousResponseID(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
...
func TestCodexExecutorExecuteStreamStripsPayloadSystemInstruction(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
